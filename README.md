...
`tags_dict` maps each Stash tag to a filename template. Scenes are renamed in the order the tags are listed; each scene is processed by the first matching tag only.
